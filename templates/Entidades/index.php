...
<style>
    ...
    /* Table stretch to fill viewport without scroll */
    .entidades-index {
        display: flex;
...
<div class="entidades-index">
    ...
    <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem;">
        <div style="flex: 1;" id="mainSearchContainer" <?= (!empty($filterNombre) || !empty($filterDepartamento) || !empty($filterProvincia) || !empty($filterDistrito) || !empty($filterNivel)) ? 'hidden' : '' ?>>
            <form method="get" action="<?= $this->Url->build(['controller' => 'Entidades', 'action' => 'index']) ?>" id="searchForm" class="input-with-icon">
                ...
                <?php if (!empty($search)): ?>
                    <a href="<?= $this->Url->build(['controller' => 'Entidades', 'action' => 'index', '?' => ['per_page' => $perPage]]) ?>" id="clearSearch" class="input-clear" title="Limpiar búsqueda">
        ...
        <button id="toggleFilters" style="background: white; color: #666; border: 1px solid #ddd; padding: 0.7rem 1.5rem; border-radius: 4px; cursor: pointer; font-size: 14px; display: flex; align-items: center; gap: 0.5rem;">
            <?= (!empty($filterNombre) || !empty($filterDepartamento) || !empty($filterProvincia) || !empty($filterDistrito) || !empty($filterNivel)) ? '<i class="fa-solid fa-times"></i> CERRAR FILTROS' : '<i class="fa-solid fa-filter"></i> Filtros' ?>
        </button>
    </div>
    ...
    <div class="table-wrapper" style="background: white; border-radius: 8px; border: 1px solid #e0e0e0; overflow: hidden;">
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                ...
                <!-- Filter Row -->
                <tr id="filterRow" style="background: white; border-bottom: 2px solid #e0e0e0; display: <?= (!empty($filterNombre) || !empty($filterDepartamento) || !empty($filterProvincia) || !empty($filterDistrito) || !empty($filterNivel)) ? '' : 'none' ?>;">
                    <th style="padding: 0.5rem 1rem;">
                        <div class="input-with-icon">
                            ...
                            <?php if (!empty($filterNombre)): ?>
                                <button type="button" onclick="clearFilter('filterNombre')" class="input-clear" style="cursor: pointer;">
                                    <i class="fa-solid fa-times"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                    ...
                    <th style="padding: 0.5rem 1rem;">
                        <div class="input-with-icon">
                            <span class="input-icon"><i class="fa-solid fa-search" style="font-size: 12px;"></i></span>
                            <input type="text" id="filterDepartamento" value="<?= h($filterDepartamento ?? '') ?>" placeholder="Filtrar depto..." class="search-input form-control" style="padding: 0.5rem 0.5rem 0.5rem 2rem; font-size: 13px; border: 1px solid #ddd; border-radius: 4px;">
                            <?php if (!empty($filterDepartamento)): ?>
                                <button type="button" onclick="clearFilter('filterDepartamento')" class="input-clear" style="cursor: pointer;">
                                    <i class="fa-solid fa-times"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                    ...
                    <th style="padding: 0.5rem 1rem;">
                        <div class="input-with-icon">
                            <span class="input-icon"><i class="fa-solid fa-search" style="font-size: 12px;"></i></span>
                            <input type="text" id="filterProvincia" value="<?= h($filterProvincia ?? '') ?>" placeholder="Filtrar provincia..." class="search-input form-control" style="padding: 0.5rem 0.5rem 0.5rem 2rem; font-size: 13px; border: 1px solid #ddd; border-radius: 4px;">
                            <?php if (!empty($filterProvincia)): ?>
                                <button type="button" onclick="clearFilter('filterProvincia')" class="input-clear" style="cursor: pointer;">
                                    <i class="fa-solid fa-times"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                    ...
                    <th style="padding: 0.5rem 1rem;">
                        <div class="input-with-icon">
                            <span class="input-icon"><i class="fa-solid fa-search" style="font-size: 12px;"></i></span>
                            <input type="text" id="filterDistrito" value="<?= h($filterDistrito ?? '') ?>" placeholder="Filtrar distrito..." class="search-input form-control" style="padding: 0.5rem 0.5rem 0.5rem 2rem; font-size: 13px; border: 1px solid #ddd; border-radius: 4px;">
                            <?php if (!empty($filterDistrito)): ?>
                                <button type="button" onclick="clearFilter('filterDistrito')" class="input-clear" style="cursor: pointer;">
                                    <i class="fa-solid fa-times"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                    ...
                    <th style="padding: 0.5rem 1rem;">
                        <div class="input-with-icon">
                            <span class="input-icon"><i class="fa-solid fa-search" style="font-size: 12px;"></i></span>
                            <input type="text" id="filterNivel" value="<?= h($filterNivel ?? '') ?>" placeholder="Filtrar nivel..." class="search-input form-control" style="padding: 0.5rem 0.5rem 0.5rem 2rem; font-size: 13px; border: 1px solid #ddd; border-radius: 4px;">
                            <?php if (!empty($filterNivel)): ?>
                                <button type="button" onclick="clearFilter('filterNivel')" class="input-clear" style="cursor: pointer;">
                                    <i class="fa-solid fa-times"></i>
                                </button>
                            <?php endif; ?>
                        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function applyFilters() {
            ...
            if (filterProvincia.value) url.searchParams.set('filter_provincia', filterProvincia.value);
            else url.searchParams.delete('filter_provincia');

            if (filterDistrito.value) url.searchParams.set('filter_distrito', filterDistrito.value);
            else url.searchParams.delete('filter_distrito');

            if (filterNivel.value) url.searchParams.set('filter_nivel', filterNivel.value);
            else url.searchParams.delete('filter_nivel');
        ...
        [filterNombre, filterDepartamento, filterProvincia, filterDistrito, filterNivel].forEach(input => {
            if (input) {
