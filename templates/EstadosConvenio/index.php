 ...
 */
$this->assign('title', $title ?? 'Estados de Convenio');
?>

<style>
    .estados-convenio-index { display:flex; flex-direction:column; height: calc(100vh - var(--topbar-height) - 4rem); }
    .estados-convenio-index .table-wrapper { flex:1 1 auto; overflow-y:auto; min-height:0; }
    ...
    #editEstadoModal .modal-content .btn { font-size:16px; }
    #addEstadoModal textarea,
    #editEstadoModal textarea { min-height:120px; }
}</style>

<div class="estados-convenio-index">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2rem;">
        <h2 style="margin:0;color:#2c3e50;font-size:24px;font-weight:500;">Estados de Convenio</h2>
    ...
    <div style="display:flex;gap:1rem;margin-bottom:1.5rem;">
        <div style="flex:1;" id="mainSearchContainer" <?= !empty($filterDescripcion) ? 'hidden' : '' ?>>
            <form method="get" action="<?= $this->Url->build(['controller' => 'EstadosConvenio', 'action' => 'index']) ?>" id="searchForm" class="input-with-icon">
                <span class="input-icon"><i class="fa-solid fa-search"></i></span>
                <input type="text" name="search" id="searchInput" value="<?= h($search ?? '') ?>" placeholder="Buscar estados..." class="search-input form-control" style="border:1px solid #ddd;border-radius:4px;font-size:14px;background:white;color:#2c3e50;">
                <input type="hidden" name="per_page" value="<?= $perPage ?>">
                <?php if (!empty($search)) : ?>
                    <a href="<?= $this->Url->build(['controller' => 'EstadosConvenio', 'action' => 'index', '?' => ['per_page' => $perPage]]) ?>" class="input-clear" title="Limpiar búsqueda"><i class="fa-solid fa-times"></i></a>
                <?php endif; ?>
            </form>
        </div>
    ...
    <div class="table-wrapper" style="background:white;border-radius:8px;border:1px solid #e0e0e0;overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;">
            ...
            <tbody>
                <?php if (!empty($estados) && count($estados) > 0) : ?>
                    <?php foreach ($estados as $e) : ?>
                        <tr style="border-bottom:1px solid #f0f0f0;">
                            <td style="padding:1rem;font-size:14px;color:#2c3e50;"><?= h($e->descripcion) ?></td>
                            <td style="padding:1rem;vertical-align:middle;">
                                <div style="display:flex;align-items:center;gap:0.75rem;">
                                    <button type="button" class="action-icon" title="Editar" onclick='openEditModal(<?= json_encode($e) ?>)'><i class="fa-solid fa-pen-to-square icon-edit"></i></button>
                                    <?= $this->Form->postLink('<i class="fa-solid fa-trash icon-delete"></i>', ['action' => 'delete', $e->id_estado_convenio], ['confirm' => '¿Está seguro de eliminar este estado?', 'class' => 'action-icon', 'escape' => false, 'title' => 'Eliminar']) ?>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<div class="modal fade" id="addEstadoModal" tabindex="-1" aria-labelledby="addEstadoLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <?= $this->Form->create(null, ['url' => ['controller' => 'EstadosConvenio', 'action' => 'add']]) ?>
            <div class="modal-header">
                <h5 class="modal-title" id="addEstadoLabel">Nuevo Estado</h5>
...
<div class="modal fade" id="editEstadoModal" tabindex="-1" aria-labelledby="editEstadoLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <?= $this->Form->create(null, ['id' => 'editEstadoForm', 'url' => ['controller' => 'EstadosConvenio', 'action' => 'edit']]) ?>
            <div class="modal-header">
                <h5 class="modal-title" id="editEstadoLabel">Editar Estado</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?= $this->Form->hidden('id_estado_convenio', ['id' => 'edit-id']) ?>
...
<script>
    ...
    function openEditModal(e) {
        ...
        form.setAttribute('action', "<?= $this->Url->build(['controller' => 'EstadosConvenio', 'action' => 'edit']) ?>/" + e.id_estado_convenio);
        document.getElementById('edit-id').value = e.id_estado_convenio;
        document.getElementById('edit-descripcion').value = e.descripcion || '';
        try { new bootstrap.Modal(document.getElementById('editEstadoModal')).show(); } catch (err) { console.error(err); }
    }
