 ...
 * @var \Cake\Collection\CollectionInterface $tipos
 */
$this->assign('title', $title ?? 'Tipos de Reunión');
?>

<style>
    .tipos-index { display:flex; flex-direction:column; height: calc(100vh - var(--topbar-height) - 4rem); }
    .tipos-index .table-wrapper { flex:1 1 auto; overflow-y:auto; min-height:0; }
    ...
    #addTipoModal .modal-content .form-control, #addTipoModal .modal-content .btn, #editTipoModal .modal-content .form-control, #editTipoModal .modal-content .btn { font-size:16px; }
</style>

<div class="tipos-index">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2rem;">
        <h2 style="margin:0;color:#2c3e50;font-size:24px;font-weight:500;">Tipos de Reunión</h2>
        <button type="button" data-bs-toggle="modal" data-bs-target="#addTipoModal" style="background:#3498db;color:white;border:none;padding:0.7rem 1.5rem;border-radius:4px;cursor:pointer;font-size:14px;display:flex;align-items:center;gap:0.5rem;">
    ...
    <div style="display:flex;gap:1rem;margin-bottom:1.5rem;">
        <div style="flex:1;" id="mainSearchContainer" <?= (!empty($filterDescripcion)) ? 'hidden' : '' ?>>
            <form method="get" action="<?= $this->Url->build(['controller' => 'TiposReunion', 'action' => 'index']) ?>" id="searchForm" class="input-with-icon">
                <span class="input-icon"><i class="fa-solid fa-search"></i></span>
                <input type="text" name="search" id="searchInput" value="<?= h($search ?? '') ?>" placeholder="Buscar tipos de reunión..." class="search-input form-control" style="border:1px solid #ddd;border-radius:4px;font-size:14px;background:white;color:#2c3e50;">
                <input type="hidden" name="per_page" value="<?= $perPage ?>">
                <?php if (!empty($search)): ?>
                    <a href="<?= $this->Url->build(['controller' => 'TiposReunion', 'action' => 'index', '?' => ['per_page' => $perPage]]) ?>" class="input-clear" title="Limpiar búsqueda"><i class="fa-solid fa-times"></i></a>
                <?php endif; ?>
            </form>
        </div>
    ...
    <div class="table-wrapper" style="background:white;border-radius:8px;border:1px solid #e0e0e0;overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;">
            ...
            <tbody>
                <?php if (!empty($tipos) && count($tipos) > 0): ?>
                    <?php foreach ($tipos as $t): ?>
                        <tr style="border-bottom:1px solid #f0f0f0;">
                            <td style="padding:1rem;font-size:14px;color:#2c3e50;"><?= h($t->descripcion) ?></td>
                            <td style="padding:1rem;vertical-align:middle;">
                                <div style="display:flex;align-items:center;gap:0.75rem;">
                                    <button type="button" class="action-icon" title="Editar" onclick='openEditModal(<?= json_encode($t) ?>)'><i class="fa-solid fa-pen-to-square icon-edit"></i></button>
                                    <?= $this->Form->postLink('<i class="fa-solid fa-trash icon-delete"></i>', ['action' => 'delete', $t->id_tipo_reunion], ['confirm' => "¿Está seguro de eliminar este tipo?", 'class' => 'action-icon', 'escape' => false, 'title' => 'Eliminar']) ?>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="2" style="padding:3rem;text-align:center;color:#95a5a6;font-size:14px;">No hay tipos de reunión disponibles</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<!-- Add Modal -->
<div class="modal fade" id="addTipoModal" tabindex="-1" aria-labelledby="addTipoLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <?= $this->Form->create(null, ['url' => ['controller' => 'TiposReunion', 'action' => 'add']]) ?>
            <div class="modal-header">
                <h5 class="modal-title" id="addTipoLabel">Nuevo Tipo de Reunión</h5>
...
<!-- Edit Modal -->
<div class="modal fade" id="editTipoModal" tabindex="-1" aria-labelledby="editTipoLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <?= $this->Form->create(null, ['id' => 'editTipoForm', 'url' => ['controller' => 'TiposReunion', 'action' => 'edit']]) ?>
            <div class="modal-header">
                <h5 class="modal-title" id="editTipoLabel">Editar Tipo de Reunión</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?= $this->Form->hidden('id_tipo_reunion', ['id' => 'edit-id']) ?>
...
<script>
    ...
    function openEditModal(tipo) {
        const form = document.getElementById('editTipoForm');
        form.setAttribute('action', "<?= $this->Url->build(['controller' => 'TiposReunion', 'action' => 'edit']) ?>/" + tipo.id_tipo_reunion);
        document.getElementById('edit-id').value = tipo.id_tipo_reunion;
        document.getElementById('edit-descripcion').value = tipo.descripcion || '';
        try { new bootstrap.Modal(document.getElementById('editTipoModal')).show(); } catch(e) { console.error(e); }
    }
